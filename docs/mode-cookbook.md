...
报告优先生成 HTML,布局根据 `scenario` 和实际 Mode 组合动态选择;Markdown 作为纯文本 fallback。DocxXML 用于飞书创建;严格飞书图片上传与 block_replace 仍由后续 lark-doc media 流程执行。
...
回归覆盖是 cookbook 状态的判定依据。当前本地回归为 43 步;新增 mode、解析契约或报告边界声明时必须补 fixture 或专项回归步骤。
